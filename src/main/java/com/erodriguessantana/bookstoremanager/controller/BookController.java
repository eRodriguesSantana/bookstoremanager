...
public class BookController {
	...
	@ApiResponses(value = {
		    @ApiResponse(code = 200, message = "Retorna um Book pelo ID"),
	...
	@ApiResponses(value = {
		    @ApiResponse(code = 200, message = "Retorna um Book com Author pelo ID"),
		    @ApiResponse(code = 403, message = "Você não tem permissão para acessar este recurso"),
		    @ApiResponse(code = 500, message = "Foi gerada uma exceção"),
		})
	...
	public ResponseEntity<?> update(@RequestBody BookDTO bookDTO, @PathVariable Long id) {
		try {
			if (bookService.findBookById(id) != null) {
				...
				return new ResponseEntity<>("ID do Book ou ID do Author não existe.",
